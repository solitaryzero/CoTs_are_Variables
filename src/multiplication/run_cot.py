...
def run(
    ...
    # Train
    if (args.do_train):
        dataset = process_cot_dataset(train_dataset, args.cot_type)
...
def main(args):
    if (args.run_all):
        ...
        da, db = parse_dataset_name(args.data_path)
        ...
        out_path = os.path.join(args.save_result_path, f'{da}_mul_{db}_predictions.json')
        with open(out_path, 'w', encoding='utf-8') as fout:
...
if __name__ == '__main__':
    ...
    # Misc
    parser.add_argument('--cot_type', choices=['full', 'compressed'], default='full')
